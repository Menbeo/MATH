...
class Gui():
    ...
    def __init__(self, coords):
        ...
        pygame.display.set_caption("Pathfinding Algorithms Compare")
    ...
    def place_check_point(self, index):
        ...
        if (coords != self.coords.start and coords != self.coords.end
            ...
            while len(self.coords.check_points) <= int(index)-1:
                self.coords.check_points.append(None)  # Changed from "None" to None
    ...
    def remove(self):
        ...
        if coords in self.coords.walls:
            ...
            self.coords.check_points[self.coords.check_points.index(coords)] = None
    ...
    def run_algorithm(self, key):
        ...
        if len([p for p in self.coords.check_points if p is not None]) > 1:

            # create the maze array and remove missed checkpoint numbers
            self.coords.create_maze(self)
            check_points = [p for p in self.coords.check_points if p is not None]
